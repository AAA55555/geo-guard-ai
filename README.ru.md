...
|---|---|
| **Запуск команды** | `geo-guard claude …` (обычно через alias `claude`) сначала проверяет страну, и только если она разрешена — запускает Claude Code. То же самое для `cursor-agent` |
...
| `cursor-agent` (терминал) | alias `cursor-agent` → обёртку `geo-guard cursor-agent`, плюс тот же hook в `~/.cursor/hooks.json` |
...
- запуском `claude` или `cursor-agent` в обход alias (`geo-guard`-обёртки),
...
5. добавить ли **alias `claude` → `geo-guard claude`** в rc текущего shell (дефолт да);
6. добавить ли **alias `cursor-agent` → `geo-guard cursor-agent`** — спрашивается, только если `cursor-agent` есть в `PATH` (дефолт да);
7. в какой **shell** класть alias'ы — определённый автоматически предлагается по умолчанию.
...
Чужие alias вроде `cc` / `c`, если они уже заняты не нами, тоже не перезаписываются: setup берёт первое свободное из `claude`, `cc`, `ccg`, `geoclaude` и говорит, какое взял и почему.
...
У терминального клиента Cursor свой блок, со своими маркерами:
...
Ставится, только если `cursor-agent` действительно есть в `PATH`: вешать alias на команду, которой у тебя нет, — значит заменить честное «command not found» от shell на нашу собственную ошибку. Управляется флагами `--cursor-alias` / `--no-cursor-alias`; `--no-alias` означает «не лезь в мой rc» и покрывает оба alias'а.
...
**В отличие от `claude`, коллизию имени здесь мы не обходим.** Если `cursor-agent` уже кем-то определён, setup пропускает alias и говорит об этом. Запасное имя встало бы перед командой, которую никто не набирает, — выглядело бы установленным и не гейтило бы ничего.

Всё остальное — ровно как выше: блоки независимы, поэтому твои флаги в одном переживают любые изменения другого, чужое содержимое между любой парой маркеров не трогается, а повторный `setup`, которому нечего менять, вообще не притрагивается к файлу.
...
| `--force-alias` | перезаписать **наш** alias-блок, который ты правил (по умолчанию — сохраняется). Чужое содержимое между маркерами он не трогает никогда: бэкапа rc-файла нет, а geo-guard не удаляет то, чего не писал |
...
| `--alias` / `--no-alias` | ставить/не ставить alias'ы (`--no-alias` покрывает оба) |
| `--cursor-alias` / `--no-cursor-alias` | ставить/не ставить alias `cursor-agent` (по умолчанию: включён, если `cursor-agent` есть в `PATH`) |
...
- оба маркер-блока alias (`# >>> geo-guard-ai begin >>>` … и `# >>> geo-guard-ai cursor-agent begin >>>` …) во всех известных rc, причём каждый оценивается сам по себе — правка руками в одном не повод оставлять второй;
...
Alias cursor-agent: /Users/me/.zshrc
  ✅ alias 'cursor-agent' → geo-guard cursor-agent
...
- оба блока alias в rc-файле (про `cursor-agent` пишется «alias не нужен», если такой команды здесь нет): наш, наш с твоими флагами (это нормально), чужое содержимое внутри наших маркеров (это уже не нормально) — либо блок, у которого снесли закрывающий маркер. Последнее сообщается как есть, с уточнением, какой из двух случаев: наш блок `setup` починит, а блок с твоим содержимым он не тронет и чинить придётся руками;
...
`npm test` гоняет `test/*.test.cjs` — они разложены по областям (`config`, `alias`, `hooks`, `check`, `setup`, `status`, `core`), плюс `hook-invariants`: он на паре десятков форм `settings.json` и `hooks.json` проверяет, что мы никогда не удаляем чужую запись и никогда не переписываем файл, из которого ничего не вынули.

`scripts/windows-smoke.ps1` закрывает то, что двум `sh`-скриптам выше недоступно: powershell-ветку alias, `%APPDATA%` для конфига, разрешение через `PATHEXT` и запуск `.cmd`, чтение hook-payload из stdin и `status`. Ему нужен Windows, поэтому его гоняет CI — см. ниже.
