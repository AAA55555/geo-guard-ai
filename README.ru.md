...
`npm test` гоняет `test/*.test.cjs` — они разложены по областям (`config`, `alias`, `hooks`, `check`, `setup`, `core`), плюс `hook-invariants`: он на паре десятков форм `settings.json` и `hooks.json` проверяет, что мы никогда не удаляем чужую запись и никогда не переписываем файл, из которого ничего не вынули.
