...
Провайдер должен отдавать двухбуквенный ISO-код страны текстом (`ES`). Ответ не в формате `^[A-Za-z]{2}$` игнорируется. Пустой список `providers` (`[]`) означает «провайдеров нет» → страна не определяется → блок.
